...
export async function fetchOrgEscrowsList(
  ...
  if (!data || typeof data !== 'object' || !Array.isArray((data as OrgEscrowsListResponse).items)) {
  ...
  return {
    ...
    page_size: typeof row.page_size === 'number' ? row.page_size : opts.pageSize ?? 20,
...
export async function fetchOrgEscrowReportSummary(
  ...
  const row = data as OrgEscrowReportSummary & { volume_over_time?: OrgEscrowReportSummary['volume_over_time'] }
  return {
    ...row,
    volume_over_time: Array.isArray(row.volume_over_time) ? row.volume_over_time : [],
  }
}

export async function fetchOrgEscrowDetail(accessToken: string, escrowId: string): Promise<OrgEscrowDetail> {
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/detail`, {
    headers: {
      Accept: 'application/json',
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  if (!data || typeof data !== 'object' || typeof (data as OrgEscrowDetail).escrow_id !== 'string') {
...
export async function fetchOrgEscrowEvents(accessToken: string, escrowId: string): Promise<OrgEscrowEventsResponse> {
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/events`, {
    headers: {
      Accept: 'application/json',
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  if (!data || typeof data !== 'object' || !Array.isArray((data as OrgEscrowEventsResponse).events)) {
...
export async function fetchOrgEscrowHealth(accessToken: string, escrowId: string): Promise<OrgEscrowHealth> {
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/health`, {
    headers: {
      Accept: 'application/json',
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  if (!data || typeof data !== 'object' || typeof (data as OrgEscrowHealth).escrow_id !== 'string') {
...
export async function fetchOrgEscrowWebhookLogs(
  ...
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/webhooks`, {
    headers: {
      Accept: 'application/json',
      Authorization: `Bearer ${accessToken}`,
    },
...
export async function postOrgEscrowCreate(
  ...
  if (!data || typeof data !== 'object' || typeof (data as OrgEscrowCreateResponse).id !== 'string') {
    throw new Error('Unexpected create org escrow response.')
  }
  return data as OrgEscrowCreateResponse
}

export async function postOrgEscrowCancel(accessToken: string, escrowId: string): Promise<void> {
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/cancel`, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
...
export async function postOrgEscrowResend(accessToken: string, escrowId: string): Promise<void> {
  const res = await fetch(`/api/me/org-escrows/${encodeURIComponent(escrowId)}/resend`, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
