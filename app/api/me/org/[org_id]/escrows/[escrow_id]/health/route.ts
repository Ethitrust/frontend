...
/** BFF for `GET /api/v1/organizations/{org_id}/escrows/{escrow_id}/health`. */
export async function GET(request: Request, ctx: RouteContext) {
  const { org_id, escrow_id } = await ctx.params
  return proxyV1GetJson(
    request,
    `/api/v1/organizations/${encodeURIComponent(org_id)}/escrows/${encodeURIComponent(escrow_id)}/health`,
  )
}
