...
/** BFF for `POST /api/v1/organizations/{org_id}/escrows/{escrow_id}/resend`. */
export async function POST(request: Request, ctx: RouteContext) {
  const { org_id, escrow_id } = await ctx.params
  return proxyV1PostJson(
    request,
    `/api/v1/organizations/${encodeURIComponent(org_id)}/escrows/${encodeURIComponent(escrow_id)}/resend`,
  )
}
