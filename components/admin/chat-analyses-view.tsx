    ...
    <div className="space-y-4">
      {analyses.map((a) => (
        ...
        >
          ...
          {Array.isArray(a.detected_intents) && a.detected_intents.length > 0 && (
            <div className="space-y-1">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
                Detected Intents
              </p>
              <div className="flex flex-wrap gap-1.5">
                {a.detected_intents.map((intent, idx) => (
                  <span
                    key={idx}
            ...
            <details className="text-xs">
              ...
              <div className="mt-2 space-y-2">
                {a.flagged_messages.map((fm, idx) => (
                  <pre
                    key={idx}
                    className="max-h-32 overflow-auto rounded-md bg-muted/40 p-2 text-[10px] font-mono whitespace-pre-wrap"
                  >
