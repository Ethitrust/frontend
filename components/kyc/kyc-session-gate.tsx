...
export function KycSessionGate(props: {
  eyebrow?: string
  title: string
  description: string
  signedOutSubtitle?: ReactNode
  children: (accessToken: string) => ReactNode
}) {
  const token = useAuthStore((s) => s.accessToken)
  const e = ethitrustThemeTokens
  const eyebrow = props.eyebrow ?? 'Compliance'
    ...
    return (
      <div className={cn(e.layout.container, 'py-8 lg:py-12')}>
        <header className="max-w-2xl">
          ...
          {props.signedOutSubtitle ?? null}
        </header>
        <Card className="mt-10 shadow-sm">
          <CardHeader>
            <CardTitle className="text-base font-semibold">Sign in to continue</CardTitle>
            <CardDescription>Identity workflows use your authenticated Ethi-Trust session.</CardDescription>
          </CardHeader>
          <CardContent>
