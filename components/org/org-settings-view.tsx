...
export function OrgSettingsView({ orgId }: { orgId: string }) {
  ...
  const revokeMutation = useMutation({
    mutationFn: (apiKeyId: string) => deleteOrgApiKey(accessToken!, orgId, apiKeyId),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['me', 'organizations', orgId, 'api-keys'] }),
    onError: (err: unknown) =>
      toast.error(err instanceof Error ? err.message : 'Could not revoke API key'),
  })

  function confirmRevoke(id: string, name: string) {
    if (
      typeof window !== 'undefined' &&
      !window.confirm(`Revoke API key "${name}"? This cannot be undone from the dashboard.`)
    )
      return
  ...
  return (
    <div className={cn(e.layout.container, 'py-8 lg:py-12')}>
      <header className="max-w-2xl">
        ...
        <h1 className={cn(e.typography.displayLG, 'mt-2 font-serif font-normal text-foreground')}>Settings</h1>
        <p className={cn(e.typography.bodyMuted, 'mt-3')}>
          Profile from <span className="font-mono text-xs">GET …/organizations/{'{org_id}'}/profile</span>; API keys
          from <span className="font-mono text-xs">GET …/api-keys</span>.
        </p>
      </header>
        ...
        <Alert variant="destructive" className="mt-10">
          ...
          <AlertDescription className="space-y-2">
            {profileQuery.isError ? (
              <span className="block">{profileQuery.error instanceof Error ? profileQuery.error.message : ''}</span>
            ) : null}
            {keysQuery.isError ? (
              <span className="block">{keysQuery.error instanceof Error ? keysQuery.error.message : ''}</span>
            ) : null}
          </AlertDescription>
        </Alert>
      )}
          ...
          {(profile.is_flagged || profile.kyb_status !== 'verified') && (
            <Alert variant={profile.is_flagged ? 'destructive' : 'default'}>
              ...
              <AlertDescription className="text-sm">
                <span className="font-medium">kyb_status:</span> {profile.kyb_status}
                {profile.is_flagged ? (
                  <>
                    {' '}
                ...
                {profile.risk_score > 0 ? (
                  <>
                    {' '}
                    · <span className="font-medium">risk_score:</span> {profile.risk_score}
                  </>
                ) : null}
              </AlertDescription>
            </Alert>
          ...
          <Card className="shadow-sm">
            <CardHeader className="flex-row flex-wrap items-center justify-between gap-4 space-y-0 border-b">
              <div>
                ...
                <CardDescription>
                  <span className="font-mono text-xs">POST …/api-keys</span> with{' '}
