...
import {
  fetchOrgWalletList,
  fetchOrgWalletTransactions,
} from '@/lib/org/org-wallet-api'
import { pickDefaultWalletId } from '@/lib/wallets/me-wallet-api'
...
export function OrgWalletTransactionsView({
  orgId,
  initialWalletId,
}: {
  orgId: string
  ...
  return (
    <WalletPaymentsGate
      title="Org transaction history"
      description="Review deposits, withdrawals, escrow movements, and other entries for this organization's wallets."
    >
      {(accessToken) => (
        <OrgWalletTransactionsSignedIn
...
function OrgWalletTransactionsSignedIn({
  accessToken,
  orgId,
  initialWalletId,
}: {
  ...
  const txsQuery = useQuery({
    queryKey: ['org', orgId, 'wallet', walletId, 'transactions', page, PAGE_SIZE],
    queryFn: () =>
      fetchOrgWalletTransactions(accessToken, orgId, walletId, page, PAGE_SIZE),
  ...
  return (
    <WalletFlowShell
      ...
      description="Financial movements linked to this organization's wallets. Switch wallets when balances exist in multiple currencies."
          ...
          {walletsQuery.data.length > 1 ? (
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div className="space-y-2">
                <label className="text-sm font-medium" htmlFor="org-tx-wallet-select">
                  Wallet
                </label>
                <Select value={walletId} onValueChange={setWalletId}>
                  <SelectTrigger
                    id="org-tx-wallet-select"
              ...
              <Button
                ...
                asChild
              >
                <Link href={`/org/${orgId}/wallet`}>Overview</Link>
              </Button>
            </div>
          ) : null}
                        ...
                        <TableCell className="align-top">
                          {row.escrow_id ? (
                            <Link
                              ...
                              href={`/org/${orgId}/escrows/${row.escrow_id}`}
                            >
                              Escrow
                            </Link>
                          ) : null}
