...
import { fetchModeratorAssignedDisputes } from '@/lib/admin/admin-platform-api'
...
export function ModeratorDisputesAssignedView({ accessToken }: { accessToken: string }) {
  const e = ethitrustThemeTokens
  const [page, setPage] = useState(1)
  const pageSize = 20

  const listQuery = useQuery({
    queryKey: ['moderator', 'disputes', 'assigned', page, pageSize],
    queryFn: () => fetchModeratorAssignedDisputes(accessToken, page, pageSize),
    ...
    <div className={cn(e.layout.container, 'py-8 lg:py-12')}>
      <header className="max-w-2xl">
        ...
        <p className={cn(e.typography.bodyMuted, 'mt-3')}>
          Disputes assigned to you for mediation. Open the console to review the thread, add notes, and
          apply actions.
        </p>
      </header>
      ...
      <Card className="mt-10 shadow-sm">
        ...
        <CardContent>
          {listQuery.isPending ? (
            ...
            <p className="text-sm text-muted-foreground">No assigned disputes on this page.</p>
          ) : (
                  ...
                  {(items as AdminDisputeListRow[]).map((row) => (
                    <TableRow key={row.dispute_id}>
                      ...
                      <TableCell className="align-top font-mono text-xs">
                        <Link
                          href={`/moderator/disputes/${encodeURIComponent(row.escrow_id ?? '')}`}
                      ...
                      <TableCell className="text-right align-top">
                        <Button size="sm" variant="outline" className="rounded-full" asChild>
                          <Link href={`/moderator/disputes/${encodeURIComponent(row.dispute_id)}`}>
                            Console
                          </Link>
                        </Button>
                      </TableCell>
